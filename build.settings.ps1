...
Properties {
    ...
    $ModuleName = Get-Item $SrcRootDir/*.psd1 |
        Where-Object { $null -ne (Import-PowerShellDataFile -Path $_ -ErrorAction SilentlyContinue) } |
    ...
    $InstallPath = Join-Path (Split-Path $(if ($profile) {$profile}
                             ...
                             "Modules\$ModuleName\$((Import-PowerShellDataFile -Path $SrcRootDir\$ModuleName.psd1).ModuleVersion.ToString())"
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $CodeCoverageFiles = "$SrcRootDir\*.psm1"
    ...
    # Exclude these tags from pester tests
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    [string[]]$excludeTestTags = 'QE'
...
Task InlineModules -requiredVariables ModuleOutDir, ModuleName {
    ...
    $psm1Content = Get-Content -LiteralPath $psm1Path
    ...
    Select-String -LiteralPath $psm1Path -Pattern 'using module .\\([\w\.]+)' | Foreach-Object {
        ...
        $psm1Content[$line] = $c.Where( {$_ -notlike 'using module*'}) -join [Environment]::NewLine
        $using += $u
        
        Remove-Item -LiteralPath $ModuleOutDir/$fileName            
    }
    $using,$psm1Content = $psm1Content.Where( {$_ -match '^using namespace'}, 'Split')    
...
Task AfterStageFiles -depends InlineModules {
